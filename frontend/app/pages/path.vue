      ...
      <section class="panel">
        <div class="grid main-aside">
          <div>
            ...
            <h2>{{ t(roleName(store.profile.roleId)) }}</h2>
            <div class="path-line">
              <div
              ...
              >
                <div class="node">{{ t(index + 1) }}</div>
                <strong>{{ t(grade.name) }}</strong
                ><small>{{
                  t(
      ...
      <div class="grid main-aside section">
        <section class="panel">
          ...
          <p v-if="!store.trajectory.gaps.length" class="empty-inline">
            {{ t("Требования не заданы.") }}
          </p>
          <CqNotice>{{
            t(
              "100% означает соответствие навыкам. Повышение подтверждается отдельным решением компании.",
            )
          }}</CqNotice>
        </section>
        <div class="stack">
          <section class="panel">
            ...
            <CqNotice color="gold">{{
              t(
                "Изменение цели пока недоступно. Обратитесь к HR для актуализации профиля. Траектория строится к следующему грейду текущей роли.",
              )
            }}</CqNotice>
          </section>
          <section class="panel">
            ...
            <p class="small muted section">
              {{
                t(
                  "Сервер делит сумму min(текущий уровень, требование) на сумму положительных требований и умножает на 100%. Если данных недостаточно, процент не показывается.",
                )
              }}
            </p>
          </section>

...
afterEach(() => { setLocale("ru", false); localeState.explicit = false; });
...
test("persisted preference restores, other tabs sync, unavailable storage is safe", () => {
  ...
  try {
    ...
    adoptPreferredLocale("kk");
    assert.equal(locale.value, "pt", "another tab's explicit selection wins over profile default");
    listeners.storage({ key: LOCALE_STORAGE_KEY, newValue: null });
    assert.equal(localeState.explicit, false);
    adoptPreferredLocale("kk");
    assert.equal(locale.value, "kk");
...
test("seven UI languages map to supported API locales and preserve an explicit profile override", () => {
  for (const { code } of languages) {
    setLocale(code, false);
    assert.equal(apiLocale(), ["ru", "en", "kk"].includes(code) ? code : "en");
  }
  localeState.explicit = false;
  adoptPreferredLocale("kk");
  assert.equal(locale.value, "kk");
  assert.equal(localeState.explicit, false);
  setLocale("de");
  adoptPreferredLocale("ru");
  assert.equal(locale.value, "de");
  assert.equal(apiLocale(), "en");
});

test("prototype-like imported text remains text in every language and inherited catalogue data is ignored", () => {
  for (const { code } of languages) {
    setLocale(code, false);
    for (const key of ["__proto__", "constructor", "toString", "hasOwnProperty"]) {
      assert.equal(t(key), key);
      assert.equal(catalogText({ title: key }, "title"), key);
    }
    const inherited = Object.create({ [code]: { title: "inherited" } });
    assert.equal(catalogText({ title: "custom", translations: inherited }, "title"), "custom");
  }
});

test("plain errors retain diagnostics while HTTP errors remain localized and prototype-safe", () => {
  setLocale("en", false);
  assert.equal(uiError(new Error("storage unavailable")), "storage unavailable");
  assert.match(t(uiError({ status: 503, code: "TEST_UNAVAILABLE", message: "storage unavailable" })), /TEST_UNAVAILABLE/);
  assert.doesNotMatch(t(uiError({ status: 503, code: "TEST_UNAVAILABLE", message: "storage unavailable" })), /storage unavailable/);
  assert.match(t(uiError({ status: "__proto__", message: "unknown error" })), /__proto__/);
  assert.equal(uiError(new Error("constructor")), "constructor");
});

test("multifactor v2 explanation retains critical skill, sequential level, relevant drops and recency facts", () => {
  const rec = { factors: [
    { category: "SKILL_GAP", facts: { skillId: "SQL", currentLevel: 2, requiredLevel: 4, actualGain: 0.125, nextLevel: 2.125, critical: true } },
    { category: "PARTICIPATION_HISTORY", reasonCode: "OBSERVED_RELEVANT_HISTORY", facts: {
      relevantCompleted: 2, relevantMissedOrDeclined: 0, relevantDropped: 3,
      sameFormatCompleted: 999, sameFormatMissedOrDeclined: 999,
      weightedCompleted: 0.75, weightedUnfinished: 0.5, recencyHalfLifeDays: 180,
    } },
  ] };
  const original = JSON.stringify(rec);
  setLocale("en", false);
  const explanation = explainRecommendation(rec);
  assert.match(explanation, /SQL: level 2, requirement 4; expected gain 0.125/);
  assert.match(explanation, /critical skill/i);
  assert.match(explanation, /2.125/);
  assert.match(explanation, /3/);
  assert.match(explanation, /180/);

import { Controller, Get, HttpCode, Param, Post, UploadedFiles, UseInterceptors } from '@nestjs/common';
...
export class ImportController {
  ...
  async dryRun(@UploadedFiles() files: Record<string, UploadedDatasetFile[]>, @CurrentUser() actor: Actor) { return { data: await this.imports.run(filesFromUpload(files), true, actor.id) }; }
  @Post()
  @UseInterceptors(upload)
  @ApiConsumes('multipart/form-data')
  @ApiBody({ schema: multipartSchema })
  ...
  async apply(@UploadedFiles() files: Record<string, UploadedDatasetFile[]>, @CurrentUser() actor: Actor) { const result = await this.imports.run(filesFromUpload(files), false, actor.id); if (result.status === 'REJECTED') throw new DomainError('IMPORT_REJECTED', 'Package was rejected without business-data changes', 422, result); return { data: result }; }
  @Get(':importId')

...
import type { DatasetParser, ImportFiles, ImportOptions, ParsedImport } from '../../application/import.service';
import { activitiesSchema, employeesSchema, historySchema, rulesSchema, skillsSchema } from '../schemas/dataset.schemas';
import { normalizeJury } from './jury-normalizer';
...
export class JsonCsvDatasetParser implements DatasetParser {
  ...
  parse(files: ImportFiles, defaults?: DatasetRules, options: ImportOptions = {}): ParsedImport {
    ...
    const jury = options.mode === 'jury' && z.string().uuid().safeParse(options.namespace).success ? normalizeJury(data, options.namespace!, rules.asOfDate, diagnostics) : undefined;
    if (options.mode === 'jury' && !jury) diagnostics.push({file:'',field:'namespace',code:'INVALID_JURY_NAMESPACE',message:'Jury imports require a stable UUID namespace for preview and application.'});
    ...
    const isPartial = !['skills.json','employees.json','events.json','activity_history.csv'].every(file => Object.hasOwn(files,file));
    const requiredSnapshot = defaults && (isPartial || jury) ? defaults.asOfDate : undefined;
    if (requiredSnapshot && rules.asOfDate !== requiredSnapshot) diagnostics.push({file:'',field:'meta.as_of_date',code:'PARTIAL_SNAPSHOT_CONFLICT',message:`Partial and jury imports must use the existing snapshot ${requiredSnapshot}. A different date would alter availability and HR calculations for all employees; use a separate dataset/database or a full reviewed import.`});
    ...
    return { plan, fileHashes, requiredSnapshot, report: { valid: diagnostics.length === 0, diagnostics, counts: { create: 0, update: 0, skip: 0, conflict: 0 }, records: { skills: plan.skills.length, grades: plan.grades.length, employees: plan.employees.length, activities: plan.activities.length, history: plan.history.length }, rules, ...(jury ? {jury} : {}) } };
  }
}

...
function view(row: Employee) { return { id: row.id, fullName: row.fullName, roleId: row.roleId, gradeId: row.gradeId, department: row.department, managerId: row.managerId, tenureMonths: row.tenureMonths, workFormat: row.workFormat, preferredLanguage: row.preferredLanguage, hireDate: row.hireDate.toISOString().slice(0, 10), lastReviewDate: row.lastReviewDate.toISOString().slice(0, 10), careerGoal: row.careerGoal as { target_role: string; target_grade: string } | null, version: row.version }; }
@Injectable()
export class PrismaEmployeeReader implements EmployeeReader {
  ...
  async list(filters: EmployeeFilters) { const where = { roleId: filters.roleId, gradeId: filters.gradeId, department: filters.department }; const [rows, total] = await Promise.all([this.prisma.employee.findMany({ where, orderBy: { id: 'asc' }, skip: (filters.page - 1) * filters.pageSize, take: filters.pageSize }), this.prisma.employee.count({ where })]); return { data: rows.map(view), meta: { page: filters.page, pageSize: filters.pageSize, total } }; }
}

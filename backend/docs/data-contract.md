...
**Безопасная проверка альтернативного профиля жюри с уже существующим ID:** создать копию профиля с новым уникальным `employee_id` (например, `jury_case_1`), обновить `employee_id` во всех относящихся к этой копии строках истории и назначить копируемым строкам новые уникальные `record_id`. Загрузить профиль и всю нужную историю вместе, сначала через dry-run. Исходный профиль, его история и ledger сохраняются. Этот сценарий сравнивает альтернативный проверочный профиль; он не является переаттестацией реального сотрудника. Диагностики `BASELINE_CONFLICT` и `BACKDATED_HISTORY_CONFLICT` содержат эти действия и ID, вызвавший конфликт. Нельзя менять baseline mode или даты для обхода защиты.

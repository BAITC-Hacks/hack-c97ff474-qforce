...
Все пути в таблице начинаются с `/api/v1`, если не указано обратное. «Подключено» означает использование реального endpoint; наличие endpoint само по себе не означает подключение всех возможных его вариантов.
...
| Обзор, `/recommendations` | Последние сохраненные рекомендации | `GET /employees/{id}/recommendations/latest?locale=ru` | Подключено | `data: null` означает, что набор еще не создан. Чтение не вызывает модель. |
| `/recommendations` | Создать или обновить рекомендации | `POST /employees/{id}/recommendations` | Подключено | Сохраненный свежий набор может переиспользоваться. `force: true` принудительно генерирует новый. Не более трех рекомендаций. |
...
| `/event?id=...` | Карточка активности | `GET /activities/{activityId}?locale=ru` | Подключено | Подробности и даты приходят из каталога; неизвестный ID — 404. |
...
| `/event`, рекомендации | Записаться | `POST /employees/{id}/participations` | Подключено | Передается `activityId`, при выборе даты — `sessionDate`. Повторная запись может вернуть существующее участие. |
...
| `/hr/needs-attention` | Массив `{employeeId,roleId,gradeId,reasons,recordedParticipations,skippedParticipations}` | Даты, страницы, `interpretation: OBSERVABLE_SIGNALS_ONLY` |
...
Причины внимания: `INCOMPLETE_REQUIREMENTS_OR_LEVELS`, `NO_ELIGIBLE_ACTIVITIES`, `NO_PARTICIPATION_IN_WINDOW`, `REPEATED_SKIPS_IN_WINDOW`. Ни одна не означает автоматически «выгорел», «немотивирован» или «уволится».
...
Финальный Playwright-прогон: 12/12 сценариев сотрудника и HR прошли на production Docker-стеке. Проверены запись, завершение, обратная связь, серверная идемпотентность, сохранённый результат после перезагрузки, запрет HR-доступа, истечение сессии, ошибки, импорт и desktop/mobile. Перезапуск PostgreSQL/API/frontend сохранил профиль и историю. Подробные команды, результаты и границы: [verification.md](verification.md).

Тест внешней LLM запускается только явно с реальными настройками провайдера; в этой проверке он пропущен. Локальный HTTP-аудит использовал только явно выбранные fixtures в тестовой БД; это не скрытая подстановка в рабочем API.

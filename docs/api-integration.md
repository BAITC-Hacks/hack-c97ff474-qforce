...
Обновлено после исправлений 23.09.2026. Актуальные результаты повторной проверки и живого OpenAI: [fixes-verification.md](fixes-verification.md). Предыдущие результаты в `verification.md` относятся к состоянию до этого аудита.
...
- Переключатель интерфейса поддерживает русский и казахский. Выбранный язык сохраняется локально и передаётся в запросы каталогов и рекомендаций; при отсутствии ручного выбора учитывается язык профиля. Для неизвестного перевода каталога сохраняется исходное название.
...
| Обзор, `/recommendations`, HR-профиль | Последние сохраненные рекомендации | `GET /employees/{id}/recommendations/latest?locale=ru` | Подключено | `data: null` означает, что набор еще не создан. Чтение не вызывает модель; `locale` соответствует выбранному языку. |
